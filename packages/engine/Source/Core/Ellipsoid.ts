 ...
 * @private
 */
type RealValuedScalarFunction = (x: number) => number;

// Gauss-Legendre Quadrature constants
const abscissas: readonly number[] = [
 ...
 *
 * @param a - The lower bound for the integration.
 * @param b - The upper bound for the integration.
 * @param func - The function to integrate.
 * @returns The value of the integral of the given function over the given domain.
 *
 * @private
 */
function gaussLegendreQuadrature(
  a: number,
  b: number,
  func: RealValuedScalarFunction
): number {
...
// Scratch variables for instance methods
...
class Ellipsoid {
   ...
   */
  static readonly packedLength: number = Cartesian3.packedLength;
   ...
   *
   * @param x - The radius in the x direction. Defaults to 0.
   * @param y - The radius in the y direction. Defaults to 0.
   * @param z - The radius in the z direction. Defaults to 0.
   *
  ...
  static set default(value: Ellipsoid) {
    ...
    (Cartesian3 as any)._ellipsoidRadiiSquared = value.radiiSquared;
    (Cartographic as any)._ellipsoidOneOverRadii = value.oneOverRadii;
    (Cartographic as any)._ellipsoidOneOverRadiiSquared =
      value.oneOverRadiiSquared;
    (Cartographic as any)._ellipsoidCenterToleranceSquared =
   ...
   */
  static readonly UNIT_SPHERE = Object.freeze(
   ...
   */
  static readonly MOON = Object.freeze(
    new Ellipsoid(
   ...
   */
  static readonly MARS = Object.freeze(
   ...
   *
   * @param cartesian - The ellipsoid's radius in the x, y, and z directions. Defaults to Cartesian3.ZERO.
   * @param result - The object onto which to store the result, or undefined if a new
   *                 instance should be created.
  ...
  static fromCartesian3(cartesian?: Cartesian3, result?: Ellipsoid): Ellipsoid {
    ...
    // Re-initialize with new radii values
    const x = cartesian!.x;
    const y = cartesian!.y;
    const z = cartesian!.z;
   ...
   * @param startingIndex - The index into the array at which to start packing the elements. Defaults to 0.
   ...
   * @param startingIndex - The starting index of the element to be unpacked. Defaults to 0.
  ...
  clone(result?: Ellipsoid): Ellipsoid {
    return Ellipsoid.clone(this, result) as Ellipsoid;
  }
   ...
   *
   * @example
   * // Create a Cartographic and determine its Cartesian representation on a WGS84 ellipsoid.
   ...
   * const positions = [
   *   new Cesium.Cartesian3(17832.12, 83234.52, 952313.73),
   ...
   * @param buffer - A buffer to subtract from the ellipsoid size when checking if the point is inside the ellipsoid. Defaults to 0.
...
// Initialize the default ellipsoid
