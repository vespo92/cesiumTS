...
/**
 * Compare function for leap second binary search
 */
function compareLeapSecondDates(
  leapSecond: LeapSecond,
  dateToFind: { julianDate: JulianDate }
): number {
...
/**
 * Set the components of a JulianDate, normalizing seconds into days as needed
 */
...
/**
 * Compute Julian date components from Gregorian date
 */
...
class JulianDate {
  ...
  /**
   * Creates a new JulianDate instance.
   *
   ...
   * @param secondsOfDay - The number of seconds into the current Julian Day Number. Fractional seconds, negative seconds and seconds greater than a day will be handled correctly.
   * @param timeStandard - The time standard in which the first two parameters are defined.
   */
  constructor(
    julianDayNumber: number = 0.0,
    secondsOfDay: number = 0.0,
    timeStandard: number = TimeStandard.UTC
  ) {
    this.dayNumber = 0;
    this.secondsOfDay = 0;
  ...
  /**
   * Creates a new instance from an ISO 8601 date.
  ...
  static fromIso8601(iso8601String: string, result?: JulianDate): JulianDate {
    ...
    let tokens: RegExpMatchArray | string[] | null = iso8601String.split("T");
    let year: number;
    let month: number = 1;
    let day: number = 1;
    let hour: number = 0;
   ...
   */
  static clone(
    julianDate: JulianDate | undefined,
    result?: JulianDate
  ): JulianDate | undefined {
    if (!defined(julianDate)) {
      return undefined;
   ...
   * @returns true if the two dates are within epsilon seconds of each other; otherwise false.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns The difference, in seconds, when subtracting right from left.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns The difference, in days, when subtracting right from left.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns true if left is earlier than right, false otherwise.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns true if left is earlier than or equal to right, false otherwise.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns true if left is later than right, false otherwise.
   */
   ...
   * @param left - The first instance.
   * @param right - The second instance.
   * @returns true if left is later than or equal to right, false otherwise.
   */
   ...
   * @returns true if the two dates are within epsilon seconds of each other; otherwise false.
   */
  equalsEpsilon(right?: JulianDate, epsilon: number = 0): boolean {
...
// Initialize the leap seconds array
// This must be done after the class is defined since we create JulianDate instances
