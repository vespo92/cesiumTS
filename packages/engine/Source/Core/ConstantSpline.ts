import Cartesian3 from "./Cartesian3.js";
import Check from "./Check.js";
import DeveloperError from "./DeveloperError.js";
import Quaternion from "./Quaternion.js";
import Spline from "./Spline.js";

/**
 * Type for spline values - can be a number, Cartesian3, or Quaternion
 */
type SplineValue = number | Cartesian3 | Quaternion;

/**
 * Interface for value types that can be cloned
 */
interface CloneableType {
  clone(value: unknown, result?: unknown): unknown;
}

/**
 * A spline that evaluates to a constant value. Although this follows the {@link Spline} interface,
 * it does not maintain an internal array of times since its value never changes.
 *
 * @alias ConstantSpline
 *
 * @example
 * const position = new Cesium.Cartesian3(1.0, 2.0, 3.0);
 * const spline = new Cesium.ConstantSpline(position);
 *
 * const p0 = spline.evaluate(0.0);
 *
 * @see LinearSpline
 * @see HermiteSpline
 * @see CatmullRomSpline
 * @see QuaternionSpline
 * @see MorphWeightSpline
 */
class ConstantSpline {
  private _value: SplineValue;
  private _valueType: CloneableType | NumberConstructor;

  /**
   * Creates a new ConstantSpline.
   * @param value - The constant value that the spline evaluates to.
   */
  constructor(value: SplineValue) {
    this._value = value;
    this._valueType = Spline.getPointType(value);
  }

  /**
   * The constant value that the spline evaluates to.
   */
  get value(): SplineValue {
    return this._value;
  }

  /**
   * Finds an index i in times such that the parameter
   * time is in the interval [times[i], times[i + 1]].
   *
   * Since a constant spline has no internal times array, this will throw an error.
   *
   * @param time - The time.
   * @throws findTimeInterval cannot be called on a ConstantSpline.
   */
  findTimeInterval(time: number): never {
    //>>includeStart('debug', pragmas.debug);
    throw new DeveloperError(
      "findTimeInterval cannot be called on a ConstantSpline.",
    );
    //>>includeEnd('debug');
  }

  /**
   * Wraps the given time to the period covered by the spline.
   *
   * @param time - The time.
   * @returns The time, wrapped around to the updated animation.
   */
  wrapTime(time: number): number {
    //>>includeStart('debug', pragmas.debug);
    Check.typeOf.number("time", time);
    //>>includeEnd('debug');

    return 0.0;
  }

  /**
   * Clamps the given time to the period covered by the spline.
   *
   * @param time - The time.
   * @returns The time, clamped to the animation period.
   */
  clampTime(time: number): number {
    //>>includeStart('debug', pragmas.debug);
    Check.typeOf.number("time", time);
    //>>includeEnd('debug');

    return 0.0;
  }

  /**
   * Evaluates the curve at a given time.
   *
   * @param time - The time at which to evaluate the curve.
   * @param result - The object onto which to store the result.
   * @returns The modified result parameter or the value that the constant spline represents.
   */
  evaluate<T extends Cartesian3 | Quaternion>(
    time: number,
    result?: T,
  ): SplineValue {
    //>>includeStart('debug', pragmas.debug);
    Check.typeOf.number("time", time);
    //>>includeEnd('debug');

    const value = this._value;
    const ValueType = this._valueType;

    if (ValueType === Number) {
      return value;
    }

    return (ValueType as CloneableType).clone(value, result) as SplineValue;
  }
}

export default ConstantSpline;
